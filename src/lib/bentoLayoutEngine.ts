...
export function getBentoGridClass(colSpan: number): string {
  ...
  if (colSpan >= 8) return `col-span-12 lg:col-span-${colSpan}`;
  if (colSpan >= 6) return `col-span-12 lg:col-span-${colSpan}`;
  return `col-span-12 sm:col-span-6 lg:col-span-${colSpan}`;
}
  ...
  while (remaining.length > 0) {
    ...
    if (spaceLeft < 4) {
      if (currentRow.length > 0) {
        // 如果当前行还有多余空隙 (如 2 列)，并且该行中有 flexible 的组件，拉伸该组件补齐 12 列消除缝隙
        if (spaceLeft > 0) {
...
export function solveDesktopModularLayout(
  ...
  // 主循环：单元格逐行逐列扫描 (Cell-by-cell scan with Lookahead Gap-Filling)
  let r = 0;
